...
var Queue = function(options) {
  ...
  this.callbackHandler = function(context, id){
      ...
      winston.debug("Callback invoked: ", {id: id});
      if(item) {
        ...
        var options = {
          ...
          method: 'POST',
          headers: {"XDEBUG-SESSION": 'phpstorm-xdebug'}
        };
        winston.debug("Callback options", {options: options});
        req = http.request(options, function(res){
          winston.debug("Callback response received.", {
          ...
          res.addListener('end', function(){
            if(res.statusCode != 200) {
              winston.warn("Callback did not return 200 status code.", { item: item });
            }
          });
        });
        ...
        req.addListener('error', function(e){
          if (e.errno === process.ECONNREFUSED) {
            winston.error('Callback connection refused', { item: item });
          } else {
            winston.error('Callback connection error', { item: item } );
          }
        });
